        ...
        if model == 'normal':
            ...
            for i in range(self.time_interval):
                for j in range(param):
                    ...
                    temp_job.metadata['name'] = f'job-{temp_index}'
                    temp_job.spec.tasks.template.metadata['lavels']['job'] = f'job-{temp_index}'
                    for temp_container in temp_job.spec.tasks.template.spec.containers:
                        temp_container.args[11] = f'gpu-test{temp_index}'
                    temp_job.metadata['labels']['sub-time'] = f"{int(temp_job.metadata['labels']['sub-time']) + i * 1000}"
            ...
            while submit_time < self.time_interval * 1000:
                ...
                for temp_container in temp_job.spec.tasks.template.spec.containers:
                    temp_container.args[11] = f'gpu-test{temp_index}'
                temp_job.metadata['labels']['sub-time'] = f"{int(temp_job.metadata['labels']['sub-time']) + submit_time}"
                random_jobs.append(temp_job)
                temp_index += 1
